...
import { getTeamDisplayColor } from './teamColors.js';
import { escapeAttr } from '../lib/escape.js';

export function teamLogo(teamAbbr, size = 24) {
  const abbr = (teamAbbr || '').toUpperCase();
  if (!abbr || abbr === '—') return '';
  // Display (luminance-floored) color: raw dark primaries vanish on the
  // dark theme. Hairline border keeps even black (LV) visible.
  const color = getTeamDisplayColor(abbr);
