...
Depth signal (honest pre-week): FantasyPros 2026 depth CSV (preseason,
gitignored, required — backtest errors clearly if absent). Names normalized
(strip Jr/Sr/II/III/IV/V + punctuation) and joined on depth-team, NOT history
team, so offseason movers resolve (Geno NYJ QB1, Kyler MIN QB1, Tua ATL QB1).
Naive history-team join mis-ranks 29/81 QBs (incl. true starters) — the join
fix is load-bearing, see Task 2.
...
DEPTH_CSV = REPO_ROOT / "FantasyPros_Fantasy_Football_2026_Depth_Charts.csv"
...
def load_depth_qb():
    """Parse depth CSV -> {normname: (abbr, rank)}. Errors if CSV absent."""
    if not DEPTH_CSV.exists():
        sys.exit(
            f"[snap2026] missing {DEPTH_CSV.name} (gitignored by design) — "
            "download the 2026 FantasyPros depth CSV to the repo root and re-run."
        )
    depth = {}
    cur = None
    with open(DEPTH_CSV, newline="", encoding="utf-8") as f:
        for row in csv.reader(f.read().splitlines()):
            ...
            if nm and cur is not None:
                depth[cur].append(nm)
    out = {}
    for full, names in depth.items():
        abbr = FULL.get(full)
        if not abbr:
            print(f"[snap2026] WARNING: depth team not mapped: {full}")
            continue
        for i, nm in enumerate(names):
            out[norm_name(nm)] = (abbr, i)
    return out
...
def run_2026():
    ...
    # QB display names per pid (latest 2025 row; depth-team join is by name).
    disp = {}
    for r in stats25:
        if r.get("position") == "QB":
            disp[str(r.get("player_id"))] = str(
                r.get("player_display_name") or r.get("player_name") or "")

    def cur_team_rank(pid):
        nm = norm_name(disp.get(pid, ""))
        return name_team_rank.get(nm, (None, 99))

    # QB1-out override weeks on depth teams (injury-matched via name->pid).
    pid_by_norm = defaultdict(list)
    for pid, nm in disp.items():
        pid_by_norm[norm_name(nm)].append(pid)
    qb1_out_weeks = set()
    for nm, (tm, rk) in name_team_rank.items():
        if rk != 0:
            continue
        for pid in pid_by_norm.get(nm, []):
    ...
    for w in played_weeks:
        ...
        for pid in sorted(set(by_out) | set(by_no)):
            ...
            dtm, drank = name_team_rank.get(nm, (None, 99)) if pos == "QB" else (None, None)
            # why depth-team-first bye filter: history team is stale for
            # offseason movers (production's team field); the depth chart is
            # the preseason-correct team. Production stale-team for movers is
            # a separate bug, out of this spec's scope.
    ...
    print(f"[snap2026] eval player-weeks: {len(actual)} "
          f"(QB {sum(qb_mask)}); depth-name hit rate: "
